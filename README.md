...
- **planejamento-estrategico-v4** — skill própria (autoria do usuário), versão modular da metodologia de planejamento estratégico: 13 módulos executáveis isoladamente ou em sequência (contexto → análise → diagnóstico → objetivo → estratégia → canais → 4 pilares → execução → mensuração → projeção → apresentação). Estava só na conta claude.ai; passou a ser versionada aqui.
